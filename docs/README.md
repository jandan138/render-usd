# Documentation Index

Welcome to the **Render USD** project documentation. This library provides a comprehensive toolkit for rendering USD (Universal Scene Description) assets using NVIDIA Isaac Sim.

## 📚 Contents

### 1. [Getting Started](getting_started.md)
*   [Prerequisites](getting_started.md#prerequisites)
*   [Installation](getting_started.md#installation)
*   [Setting up Environment](getting_started.md#setting-up-environment)

### 2. [Architecture Overview](architecture.md)
*   [System Design](architecture.md#system-design)
*   [Core Modules](architecture.md#core-modules)
*   [Directory Structure](architecture.md#directory-structure)

### 3. [Usage Guide](usage.md)
*   [Command Line Interface (CLI)](usage.md#command-line-interface)
*   [Single File Rendering](usage.md#single-file-rendering)
*   [Batch Rendering](usage.md#batch-rendering)
*   [DLC Job Submission](usage.md#dlc-job-submission)

### 4. API Reference
*   **[Core Modules](api/core.md)**: High-level rendering logic, camera management, and scene setup.
*   **[Utilities](api/utils.md)**: Helper functions for USD manipulation, image processing, and more.
*   **[Configuration](api/config.md)**: Global settings and environment variables.

### 5. [Development Guide](development.md)
*   [Contributing](development.md#contributing)
*   [Documentation Guidelines](development.md#documentation-guidelines)
*   [Testing](development.md#testing)

---

## Quick Links
*   [Source Code](../src/render_usd/)
*   [Examples](../examples/)
