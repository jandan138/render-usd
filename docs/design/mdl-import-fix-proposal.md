...
> 日期: 2026-03-11
> 状态: 调研完成，待决策
> 前置阅读: [MDL 材质修复报告](./mdl-material-fix-correction.md)

## 1. 问题回顾

GRScenes-test1 数据集中约 1.2% 的资产（~633 个，涉及 18 个类别）在 Isaac Sim GUI 中渲染为红色。

**根因**：这些资产使用的 MI_*.mdl 材质文件通过**绝对导入**（`import ::KooPbr::KooMtl;`）引用自定义 KooPbr 模块，而 KooPbr.mdl 不在 Isaac Sim 的默认 MDL 搜索路径中。详见 [mdl-material-fix-correction.md](./mdl-material-fix-correction.md)。
...
```bash
MDL_DIR="/cpfs/shared/simulation/zhuzihou/dev/usd-scene-physics-prep/GRScenes-test1/Material/mdl"
...
**推荐**：
- 如果你是 GRScenes-test1 数据集的维护者 → **方案 A**（一次修改，永久生效）
...
- [Carbonite 入门指南（本项目）](../guides/carbonite-beginner-guide.md)
